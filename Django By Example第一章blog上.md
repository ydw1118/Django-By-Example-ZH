...
这里使用第二种方法：通过重写`Manager.get_queryset()` 方法来覆盖管理器自带的Queryset，`get_queryset()` 会根据你所需要的属性返回查询集。因为`get_queryset()`返回`QuerySet`对象，可以使用`filter()`，`exclude()`和所有其他`QuerySet`方法
...
class PublishedManager(models.Manager):
    ‘’‘
    通过重写Manager.get_queryset() 方法来覆盖管理器自带的Queryset
    get_queryset() 会根据你所需要的属性返回查询集
    ’‘’
    def get_queryset(self):
        return super(PublishedManager, 
                    self).get_queryset().filter(status=1)
...
class Post(models.Model):
    ...
    注册manager到models
    ’‘’
    objects = models.Manager() # The default manager. 不写的话，只显示状态为published的内容
...
def post_list(request):
    published_posts_lists = Post.publishedset.all()
    return render(request,
                  'blog/post/list.html',
                  {'posts': published_posts_lists})
```

你刚创建了你的第一个Django视图（view）。*post_list*视图（view）将*request*对象作为唯一的参数。记住**所有的的视图（views）都有需要request这个参数**。在这个视图（view）中，我们获取到了所有状态为已发布的帖子通过使用我们之前创建的*publishedset*管理器（manager）。

最后，我们使用Django提供的快捷方法*render()*通过给予的模板（template）来渲染帖子列。这个函数将*request*对象作为参数，模板（template）路径以及变量来渲染给予的模板（template）。它返回一个渲染文本（一般是HTML代码）*HttpResponse*对象。*render()*方法考虑到了请求内容，这样任何模板（template）内容处理器设置的变量都可以带入给予的模板（template）中。你会在*第三章，扩展你的blog应用*学习到如何使用它们。
...
第一条URL模式没有带入任何参数，它映射到*post_list*视图（view）。

第二条URL模式带上了以下4个参数映射到*post_detail*视图（view）中。
...
{% block content %}
  ...
  {% for post in posts %}
    ...
    {{ post.body|truncatewords:30|linebreaks }} # 转换内容中的换行符为HTML的换行符
  {% endfor %}
{% endblock %}
```
...
* 我们使用在每页中显示的对象的数量来实例化*Paginator*类。
* 获取request中当前页数的page值给变量page   
* page值存在的情况下，通过调用*Paginator*的 *page()*方法在返回对应的page对象。
